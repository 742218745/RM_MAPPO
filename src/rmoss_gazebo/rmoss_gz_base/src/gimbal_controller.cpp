...
: node_(node), gimbal_vel_actuator_(gimbal_vel_actuator), gimbal_pos_sensor_(gimbal_pos_sensor)
{
  ...
  // lock_gimbal parameter
  node_->declare_parameter(controller_name + ".lock_gimbal", lock_gimbal_);
  node_->get_parameter(controller_name + ".lock_gimbal", lock_gimbal_);
  // sensor callback
  ...
  // timer
  // PID 更新频率 150Hz，匹配底盘控制器（real_time_factor=5.0 时 30Hz 仿真时间）
  int pid_rate = 150;
...
void GimbalController::update()
{
  ...
  if (lock_gimbal_) {
    // 锁定云台: 跳过 PID 和命令，电机速度输出为 0，云台完全静止
    cmd.yaw = 0.0;
    cmd.pitch = 0.0;
  } else {
    // pid for pitch
